...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    if (tuningState == "POWER") {
      request->send(SPIFFS, "/power.html", "text/html");
    }
    else if (tuningState == "ECO") {
      request->send(SPIFFS, "/eco.html", "text/html");
    }
    else {
      request->send(SPIFFS, "/normal.html", "text/html");
    }
  });
  server.on("/power", HTTP_GET, [](AsyncWebServerRequest * request) {
    tuningState = "POWER";
    Serial.println("Tuning mode: POWER");
    request->redirect("/");
  });
  server.on("/normal", HTTP_GET, [](AsyncWebServerRequest * request) {
    tuningState = "NORMAL";
    Serial.println("Tuning mode: NORMAL");
    request->redirect("/");
  });
    server.on("/eco", HTTP_GET, [](AsyncWebServerRequest * request) {
    tuningState = "ECO";
    Serial.println("Tuning mode: ECO");
    request->redirect("/");
  });
